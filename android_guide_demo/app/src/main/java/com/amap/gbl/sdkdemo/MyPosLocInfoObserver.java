...
/**
 * @brief      位置信息观察者
 * @details    定位引擎使用观察者模式对外通知位置信息。这里的位置信息指的是匹配后的位置信息。 \n
 *           使用者通过addLocInfoObserver接口把观察者添加到定位引擎中 \n
 *           使用者通过removeLocInfoObserver接口把观察者从定位引擎中删除。 \n
 * @attention  由于removeLocInfoObserver接口是异步的，所以外部删除后直接把观察者释放掉可能会导致定位引擎崩溃。应该先销毁定位引擎
 * @see
 */
public class MyPosLocInfoObserver implements IPosLocInfoObserver
{
    public final static String TAG = MyPosLocInfoObserver.class.getSimpleName().toString() + " ygz$";

    private TextView mTextView;

    public MyPosLocInfoObserver(TextView txtView) {
        Log.i(TAG, "MyPosLocInfoObserver");
        mTextView = txtView;
    }
    /**
     * @brief      更新位置信息
    ...
    public void onLocInfoUpdate(LocInfo locInfo) {
        //Log.i(TAG, "onLocInfoUpdate");
        //mTextView.setText("onLocInfoUpdate");
    }
